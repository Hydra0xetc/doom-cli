...
typedef struct {
    char c[8]; // for utf-8 character
    size_t c_len;
    uint32_t fg; // 0x00RRGGBB
    uint32_t bg; // 0x00RRGGBB
} term_cell_t;

static term_cell_t *current_screen = NULL;
static term_cell_t *previous_screen = NULL;
...
typedef enum keystate_t {
    ...
    keystate_wait,   // key may be down, we sent release but still waiting for a
                     // repeat
    keystate_repeat, // additional keypress received after repeat rate; key is
                     // repeating
} keystate_t;
...
const char *quadrants[] = {
    ...
    "\xE2\x96\x9B", // U+259B: QUADRANT UPPER LEFT AND UPPER RIGHT AND LOWER
                    // LEFT   ▛
    ...
    "\xE2\x96\x9C", // U+259C: QUADRANT UPPER LEFT AND UPPER RIGHT AND LOWER
                    // RIGHT  ▜
    "\xE2\x96\x84", // U+2584: LOWER HALF BLOCK ▄
    "\xE2\x96\x99", // U+2599: QUADRANT UPPER LEFT AND LOWER LEFT AND LOWER
                    // RIGHT   ▙
    "\xE2\x96\x9F", // U+259F: QUADRANT UPPER RIGHT AND LOWER LEFT AND LOWER
                    // RIGHT  ▟
...
#define LUMA_P(x)                                                              \
    (x[0] * LUMA_WEIGHT_BLUE + x[1] * LUMA_WEIGHT_GREEN +                      \
     x[2] * LUMA_WEIGHT_RED)
...
/**
 * Searches for the closest color in the given colors array and returns its
 * code.
 */
static int color_search(const uint8_t *colors,
                        size_t color_count,
                        int red,
                        int green,
                        int blue) {
...
static int color_3bit(int red, int green, int blue) {
    ...
    static const uint8_t colors[] = {
        // code     red    green     blue
        30, 0, 0, 0,   31, 255, 0, 0,   32, 0, 255, 0,   33, 255, 128, 0,
...
static int color_4bit(int red, int green, int blue) {
    ...
    static const uint8_t colors[] = {
        ...
        34, 0,   0,   170, 35, 170, 0,   170, 36, 0,   170, 170,
        37, 170, 170, 170, 90, 85,  85,  85,  91, 255, 85,  85,
        92, 85,  255, 85,  93, 255, 255, 85,  94, 85,  85,  255,
        95, 255, 85,  255, 96, 85,  255, 255, 97, 255, 255, 255,
    };
    return color_search(colors, sizeof(colors) / 4, red, green, blue);
}
...
static int color_8bit(int red, int green, int blue) {
    ...
    */
    ...
    // TODO this comment is wrong, we DON'T want to bias above the 1/8 or 1/4
    // threshold Bias grayscale based on luma. This decreases errorg in the
    // bottom 1/8 of luma and increases it otherwise. This ensures dark areas
    // are playable while basically never using grayscale in brighter ares
    // (unless the color is actually gray.)

    // printf("\033[0m%u %u   %u %u %u   %u    %u\n", error6,
    // errorg,r6,g6,b6,gray,luma);

    return ((error6 < errorg)) ? 16 + b6 + g6 * 6 + r6 * 36 : // color cube
               232 + gray;                                    // grayscale
}
...
static void output_colors(int x,
                          int y,
                          int fg_red,
                          int fg_green,
                          int fg_blue,
...
#define INT_NAME(x) x // TODO remove INT_NAME()
    ...
    switch (cli_colors) {
    case cli_colors_24bit: {
        char *p = memcpy(buf, "\033[38;2;", sizeof("\033[38;2;") - 1);
        p = stpcpy(p, u8_to_str[fg_red]);
        *p++ = ';';
        p = stpcpy(p, u8_to_str[fg_green]);
        *p++ = ';';
        p = stpcpy(p, u8_to_str[fg_blue]);
        p = memcpy(p, "m\033[48;2;", sizeof("m\033[48;2;") - 1);
...
static void output_newline(void) { buffer_append_literal("\033[0m\n"); }
...
static void draw_space() {
    ...
    for (int y = 0; y < dest_height; ++y) {
        ...
        for (int x = 0; x < dest_width; ++x) {
            ...
            // printf("\033[48;5;%um ", 16 + (pixel[0] / 43) + (pixel[1] / 43) *
            // 6 + (pixel[2] / 43) * 36);
    ...
    for (int y = 0; y < dest_height; y += 2) {
        ...
        for (int x = 0; x < dest_width; x += 2) {
            ...
            */
            ...
            int index = (l_tl > l_avg) | ((l_tr > l_avg) << 1) |
                        ((l_bl > l_avg) << 2) | ((l_br > l_avg) << 3);

            // if all bits are set, we use only background (space)
            // TODO fix this later, need to also clear luma or check bits in
            // below conditionals
            /*if (index == 0b1111) {
                index = 0;
            }*/
            ...
            */
            ...
            if (index == 0) {
                ...
                output_colors(
                    x, y, fg_red, fg_green, fg_blue, bg_red, bg_green, bg_blue);
    ...
    for (int y = 0; y < dest_height; y += 3) {
        ...
        for (int x = 0; x < dest_width; x += 2) {
            ...
            if (noise_enabled) {
                ...
                // printf("pixel x %i y %i luma %i noise raw %i noise actual
                // %i\n", x,y,l_tl,NOISE_SAMPLE(x    , y )&0xff,(NOISE_SAMPLE(x
                // , y    ) & 0xff) - 128);
                l_tl = clamp(l_tl + (NOISE_SAMPLE(x, y) & 0xff) - 128, 0, 255);
                l_tr =
                    clamp(l_tr + (NOISE_SAMPLE(x + 1, y) & 0xff) - 128, 0, 255);
                l_ml =
                    clamp(l_ml + (NOISE_SAMPLE(x, y + 1) & 0xff) - 128, 0, 255);
                l_mr = clamp(
                    l_mr + (NOISE_SAMPLE(x + 1, y + 1) & 0xff) - 128, 0, 255);
                l_bl =
                    clamp(l_bl + (NOISE_SAMPLE(x, y + 2) & 0xff) - 128, 0, 255);
                l_br = clamp(
                    l_br + (NOISE_SAMPLE(x + 1, y + 2) & 0xff) - 128, 0, 255);
            }

            // calculate index
            // printf("%i %i %i %i %i %i\n", l_tl,l_tr,l_ml,l_mr,l_bl,l_br);
            const uint32_t threshold = 127;
            int index = ((l_tl > threshold)) | ((l_tr > threshold) << 1) |
                        ((l_ml > threshold) << 2) | ((l_mr > threshold) << 3) |
                        ((l_bl > threshold) << 4) | ((l_br > threshold) << 5);
...
static void parse_cli_options() {
    ...
    if (cli_colors == cli_colors_dark || cli_colors == cli_colors_light) {
        if (cli_mode == cli_mode_space) {
            fprintf(stderr,
                    "The space charset is incompatible with light and dark "
                    "color modes.\n");
            abort();
        }
        if (cli_mode != cli_mode_sextant) {
            fprintf(stderr,
                    "TODO dark/light color mode is only implemented for "
                    "sextant charset.\n");
            abort();
        }
    }
...
void DG_Init() {
    ...
    switch (cli_mode) {
    case cli_mode_space:
        term_height = dest_height;
        break;
    case cli_mode_half:
    case cli_mode_quadrant:
        term_height = dest_height / 2;
        break;
    case cli_mode_sextant:
        term_height = dest_height / 3;
        break;
    }
    current_screen = malloc(sizeof(term_cell_t) * term_width * term_height);
    previous_screen = malloc(sizeof(term_cell_t) * term_width * term_height);
    memset(previous_screen, 0, sizeof(term_cell_t) * term_width * term_height);
...
void DG_DrawFrame() {
    ...
    //  TODO for now we just choose the nearest pixel, need to implement at
    //  least a box filter
    uint32_t *dest_pixel = dest_buffer;
    for (int y = 0; y < dest_height; ++y) {
        for (int x = 0; x < dest_width; ++x) {
            ...
            *dest_pixel++ =
                *(uint32_t *)(colors + I_VideoBuffer[sy * SCREENWIDTH + sx]);
        }
    }
    ...
    // clear the screen
    buffer_append_literal("\033[1;1H");
    ...
    if (print_stats) {
        ...
        if (stats_count < stats_capacity) {
            ...
            size_t len =
                buffer_append_format("frame size: %zi B", average_size);
            buffer_append_pad(len, 25);

            // print frame rate
            int fps = 1000 * stats_capacity /
                      (current_time - stats_times[stats_next]);
        ...
        buffer_append_format(
            "key repeat delay: %i ms    key repeat rate: %i ms\n",
            key_repeat_delay,
            key_repeat_rate);
    }

    // show the cursor
    // TODO trap ctrl+c and send this and the color reset code, this is real
    // annoying
...
static void add_key_measurement(uint32_t delay, uint32_t rate) {
    ...
    for (size_t i = 0; i < key_measure_count; ++i) {
        ...
        // printf("delay %i %i %i\n", delay, mean_delay, abs(delay -
        uint32_t delay_error = delay > mean_delay ? delay - mean_delay : mean_delay - delay;
        if (delay_error < best_delay_error) {
            best_delay = delay;
            best_delay_error = delay_error;
        }
        uint32_t rate_error = rate > mean_rate ? rate - mean_rate : mean_rate - rate;
    ...
    if (best_delay > KEY_REPEAT_DELAY_MAX)
        best_delay = KEY_REPEAT_DELAY_MAX;
    if (best_rate > KEY_REPEAT_RATE_MAX)
        best_rate = KEY_REPEAT_RATE_MAX;
...
static void detect_key_repeat(int key) {
    ...
    for (size_t i = 1; i < TIME_CAPACITY - 1; ++i) {
        uint32_t delta = keyinfo->time[(n + i + 1) % TIME_CAPACITY] -
    ...
    for (size_t i = 1; i < TIME_CAPACITY - 1; ++i) {
        uint32_t delta = keyinfo->time[(n + i + 1) % TIME_CAPACITY] -
                         keyinfo->time[(n + i) % TIME_CAPACITY];
        uint32_t error = delta > mean ? delta - mean : mean - delta;
        // printf("  error %i\n", error);
        variance += error;
    }
    // printf("variance %i\n", variance);
    ...
    uint32_t delay = keyinfo->time[(n + 1) % TIME_CAPACITY] -
...
static void handle_input_byte(int c) {
    ...
    switch (c) {
    case '\n':
        keypress(KEY_ENTER, true);
        break;
    case 'Z':
        keypress(KEY_FIRE, true);
        break;
    case ' ':
        keypress(KEY_USE, true);
        break;
    case 'X':
        keypress(KEY_LALT, true);
        break;
    case '-':
        keypress(KEY_MINUS, true);
        break;
    case '+':
    case '=':
        keypress(KEY_EQUALS, true);
        break;
    default:
        break;
    }
    ...
    // send the ascii
    // TODO this doesn't seem to be working, can't press Y/N to answer question
    // prompts
    if (c >= 0 && c <= 127) {
        keypress(c, false);
    }
...
void doomcli_read_input(void) {
    ...
    for (;;) {
        ...
        int csi = getchar();
        if (csi == -1) {
            // give it a moment to see if the rest of an escape sequence is
            // coming
            usleep(5000);
            csi = getchar();
        }
...
static void simulate_release_events(void) {
    ...
    for (int key = 0; key < sizeof(keyinfos) / sizeof(*keyinfos); ++key) {
        ...
        uint32_t expected =
            2 * key_repeat_threshold + ((keyinfo->state == keystate_repeat)
                                            ? key_repeat_rate
                                            : key_repeat_delay);

        // Check if enough time has passed for us to simulate the key release
        // event
        int last_time = keyinfo->time[(keyinfo->time_next - 1 + TIME_CAPACITY) %
                                      TIME_CAPACITY];
...
static void show_cursor(void) { puts("\033[?25h"); }
